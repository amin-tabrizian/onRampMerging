...
a = np.random.normal(size=(93)).astype(np.float32)
with torch.no_grad():   
    outputs = model(torch.from_numpy(a)).numpy()
print(outputs)
# correct = 0
# total = 0
...
#         outputs = np.ndarray.flatten(np.rint(outputs))
        
        # targets = np.ndarray.flatten(targets.numpy())
