...
sys.path.append('/home/amin/onRampMerging/')
from utils import *
...
def getState(radius, size= 99, mode= 'plain'):
    ...
    for vehID in traci.vehicle.getIDList():
        ...
        if getDistance(egoPos, vehPos) <= radius: 
            ...
            if 'f_1' in vehID:
                ...
                vehLabel = [0, 0, 1]  
            vehList = list(vehPos) + [vehVel] + vehLabel
            if  vehPos < egoPos:
    ...
    veh_ahead_list = sorted(veh_ahead_list, 
                            key= lambda x: [-x[1], -x[0]],
                            reverse=True)
    raw_state = flatten(veh_behind_list + veh_ahead_list)

    # Zero padd
    behind_padding = list(np.zeros(6*(int((size - 1)/2) - veh_behind_number)))
    ahead_padding = list(np.zeros(6*(int((size - 1)/2) - veh_ahead_number)))
...
class Merging():
    def __init__(self, options, seed, radius = 50, render_mode= None):
        ...
        self.observation_history = np.zeros((self.observation_space.shape[0], 
                                             self.delay))
