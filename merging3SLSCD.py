...
class Merging():
    def __init__(self, options, seed, radius = 50, render_mode= None, d = 1):
        ...
        self.laneID = ''
        self.d = d
        self.action_history =np.zeros((d, 2))
        ...
        low_space = np.append(low_space, np.tile(np.float32([-1, -1]), reps=self.d))
        high_space = np.tile(np.float32([1000, 1000, 50, 2]), reps= self.size)
        high_space = np.append(high_space, np.tile(np.float32([1, 1]), reps=self.d))
    ...
    def reset(self, options=None):
        ...
        return np.append(observation, np.ndarray.flatten(self.action_history))
    ...
    def step(self, action):
        ...
        return np.append(self.observation_history[return_idx], self.action_history), self.reward, self.done, {'message': info, 'lane': self.laneID, 'action': action}
